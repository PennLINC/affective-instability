...
def infotodict(
    ...
    for i_acq, curr_seqinfo in enumerate(seqinfo):
        ...
        if (
            datatype != "fmap" or datatype_suffix == "epi"
            ...
            if "P" in curr_seqinfo.image_type:
                ...
                if (next_series_description == curr_seqinfo.series_description) and (
                    next_dcm_image_iod_spec == "P"
                ):
                    series_info["part"] = "mag"

        # Need to trick DWI to handle the "part" entity correctly
        # The magnitude scan has "NORM" in the ImageType, but the phase scan doesn't.
        # The same applies to EPI field maps for the DWI scans.
        if datatype in ("dwi", "fmap") and not curr_seqinfo.series_description.endswith(
            "_SBRef"
        ):
            if (
                ("NORM" in curr_seqinfo.image_type)
                and ("NORM" not in next_seqinfo.image_type)
                and (next_series_description == curr_seqinfo.series_description)
            ):
                series_info["part"] = "mag"
            elif (
                ("NORM" not in curr_seqinfo.image_type)
                and ("NORM" in prev_seqinfo.image_type)
                and (prev_seqinfo.series_description == curr_seqinfo.series_description)
            ):
                series_info["part"] = "phase"

        # "NORM" for the anatomical scans (T1w and T2w) means rec-norm
        if (
            datatype == "anat"
            and "NORM" in curr_seqinfo.image_type
            and datatype_suffix in ("T1w", "T2w")
        ):
            series_info["rec"] = "norm"
        ...
        if datatype == "perf":
            if curr_seqinfo.series_description.endswith("_MeanPerf"):
                datatype_suffix = "cbf"
            elif curr_seqinfo.series_description.endswith("_ASL"):
                datatype_suffix = "asl"
            elif curr_seqinfo.series_description.endswith("_M0"):
                datatype_suffix = "m0scan"
        ...
        elif curr_seqinfo.is_motion_corrected:
        ...
        if (
            ...
            or "-scout_" in curr_seqinfo.series_description
            or (
                datatype == "anat"
